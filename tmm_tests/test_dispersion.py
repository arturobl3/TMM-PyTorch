...
def dispersion_sanity_test(
    dispersion: BaseDispersion, wavelengths: torch.Tensor, *, verbose: bool = False
) -> bool:
    """
    ...
    tests: List[Tuple[str, Callable[[], bool]]] = [
        ...
        (
            "Im{ñ} >= 0",
            lambda: torch.all(
                test_dispersion.refractive_index(wavelengths).imag >= 0
            ).item(),
        ),
    ]
...
def flat_dispersion_test(
    ...
    # Analytical Method
    refractiveIndex_analytical = (
        torch.ones_like(wavelengths, dtype=dtype, device=device) * refractive_index
    )
    epsilon_analytical = (
        torch.ones_like(wavelengths, dtype=dtype, device=device) * epsilon
    )

    # Dispersion method
    start = time.time()
    flat_e = Constant_epsilon(epsilon)
...
if __name__ == "__main__":
    ...
    print(
        flat_dispersion_test(
            epsilon=epsilon, wavelengths=wavelengths, dtype=dtype, device=device
        )
    )
    ...
    test_result_str = lorentz_dispersion_test(
        A, E0, C, wavelengths, dtype, device, verbose=False
    )
    ...
    # Run the test in non-verbose mode (returns a descriptive string)
    test_result_str = Cauchy_dispersion_test(
        [A, B, C, D, E, F], wavelengths, dtype, device, verbose=False
    )
    print(test_result_str)
