    ...
    *,
    ...
    for real_dtype, c_dtype in stages:
        ...
        if not _run("material.dtype correct", test_material.dtype is real_dtype):
            return False
        if not _run(
            "ε dtype correct", test_material.epsilon(wavelengths).dtype is c_dtype
        ):
            return False
        if not _run(
            "ñ dtype correct",
...
def base_material_test(
    dispersions: list[BaseDispersion],
    wavelengths: torch.Tensor,
    dtype: torch.dtype,
    device: torch.device,
    verbose: bool = False,
...
if __name__ == "__main__":
    ...
    constant_dispersion = Constant_epsilon(
        epsilon_const=epsilon_const, dtype=dtype, device=device
    )
    ...
    print(
        base_material_test(
            [constant_dispersion, Lorentz_dispersion],
            wavelengths,
            dtype,
            device,
            verbose=False,
